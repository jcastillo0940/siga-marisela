...
@section('page-title', 'Gestión de Productos y Servicios')
...
<div class="fade-in">
        ...
        <div class="stat-card bg-gradient-to-br from-green-500 to-green-600">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-white">{{ $products->where('active', true)->count() }}</p>
                </div>
                <div class="w-14 h-14 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
                    <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="stat-card bg-gradient-to-br from-blue-500 to-blue-600">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-white">{{ $products->where('track_inventory', true)->count() }}</p>
                </div>
                <div class="w-14 h-14 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
                    <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="stat-card bg-gradient-to-br from-red-500 to-red-600">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-white">{{ $products->filter(fn($p) => $p->track_inventory && $p->stock <= $p->min_stock)->count() }}</p>
                </div>
                <div class="w-14 h-14 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
                    <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="card-premium">
        ...
        <div class="text-center py-12">
            ...
            <a href="{{ route('products.create') }}" class="btn-primary mt-4 inline-flex">
                Agregar Producto
            </a>
        </div>
        @else
        <div class="overflow-x-auto">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr class="{{ $product->track_inventory && $product->stock <= $product->min_stock ? 'bg-red-50' : '' }}">
                        <td>
                        ...
                        <td class="text-center">
                            @if($product->track_inventory)
                            <span class="text-sm text-gray-600">{{ $product->min_stock }}</span>
                            @else
                            <span class="text-gray-400">-</span>
                            @endif
                        </td>
                        <td class="text-center">
                            <form action="{{ route('products.toggle-status', $product) }}" method="POST" class="inline">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="status-badge {{ $product->active ? 'status-badge-success' : 'status-badge-danger' }} cursor-pointer hover:opacity-80">

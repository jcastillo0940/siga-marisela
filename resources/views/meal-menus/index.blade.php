...
<div class="fade-in">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-4">
        ...
        <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
            <!-- Filters -->
            <form method="GET" action="{{ route('meal-menus.index') }}" class="flex items-center space-x-2">
                <select name="course_offering_id" 
                    ...
                    <option value="{{ $offering->id }}" {{ request('course_offering_id') == $offering->id ? 'selected' : '' }}>
                        {{ $offering->course->name }} - Gen {{ $offering->generation_number }}
                    </option>
                    @endforeach
                </select>
                ...
                <select name="meal_type" 
                    ...
                    <option value="breakfast" {{ request('meal_type') == 'breakfast' ? 'selected' : '' }}>Desayuno</option>
                    <option value="lunch" {{ request('meal_type') == 'lunch' ? 'selected' : '' }}>Almuerzo</option>
                    <option value="dinner" {{ request('meal_type') == 'dinner' ? 'selected' : '' }}>Cena</option>
                    <option value="snack" {{ request('meal_type') == 'snack' ? 'selected' : '' }}>Merienda</option>
                </select>
            </form>
            
            <!-- Create Button -->
            @if(auth()->user()->hasPermission('courses.create'))
            <a href="{{ route('meal-menus.create') }}" class="btn-primary text-center">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
        <div class="card-premium">
            <div class="text-center">
                ...
                <p class="text-xl sm:text-2xl md:text-3xl font-display font-semibold text-primary-dark">{{ $menus->total() }}</p>
            </div>
        </div>
        ...
        <div class="card-premium">
            <div class="text-center">
                ...
                <p class="text-xl sm:text-2xl md:text-3xl font-display font-semibold text-blue-600">
                    {{ $menus->where('meal_date', '>=', today())->count() }}
                </p>
            </div>
        </div>
        ...
        <div class="card-premium">
            <div class="text-center">
                ...
                <p class="text-xl sm:text-2xl md:text-3xl font-display font-semibold text-purple-600">
                    {{ $menus->sum(fn($m) => $m->selections->count()) }}
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="card-premium">
        @if($menus->isEmpty())
            <div class="text-center py-12">
                ...
                @if(auth()->user()->hasPermission('courses.create'))
                <a href="{{ route('meal-menus.create') }}" class="btn-primary mt-4 inline-block">
                    Crear Primer Menú
                </a>
                @endif
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div>
                                    <p class="font-medium text-primary-dark">
                                        {{ $menu->meal_date->format('d/m/Y') }}
                                    </p>
                                    <p class="text-xs text-gray-500">
                                        {{ $menu->meal_date->locale('es')->isoFormat('dddd') }}
                                    </p>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="flex items-center space-x-3">
                                    <div class="w-10 h-10 bg-purple-600 rounded-full flex items-center justify-center text-white font-medium text-sm">
                                        G{{ $menu->courseOffering->generation_number }}
                                    </div>
                                    <div>
                                        <p class="font-medium text-primary-dark text-sm">
                                            {{ $menu->courseOffering->course->name }}
                                        </p>
                                        <p class="text-xs text-gray-500">
                                            Gen {{ $menu->courseOffering->generation_number }}
                                        </p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                ...
                                @endphp
                                <span class="badge {{ $typeColors[$menu->meal_type] ?? 'badge-info' }}">
                            ...
                            <td>
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">{{ $percentage }}% completado</p>
                                </div>
                            </td>
                            ...
                            <td>
                                @if($menu->is_active)
                                    @if($menu->meal_date < today())
                                        <span class="badge badge-secondary">Pasado</span>
                                    @elseif($menu->meal_date == today())
                            ...
                            <td>
                                <div class="flex items-center justify-center space-x-2">
                                    ...
                                    <a href="{{ route('meal-menus.show', $menu->id) }}" 
                                    ...
                                    <!-- Send Notifications -->
                                    @if(auth()->user()->hasPermission('courses.edit') && $menu->meal_date >= today())
                                    <form method="POST" action="{{ route('meal-menus.send-notifications', $menu->id) }}" class="inline" id="notify-form-{{ $menu->id }}">
                                        @csrf
                                        <button type="button" 
                                                class="p-2 text-purple-600 hover:bg-purple-50 rounded transition-colors"
                                                title="Enviar notificaciones"
                                                onclick="showConfirmModal('¿Enviar recordatorios de menú a los estudiantes?', function() { document.getElementById('notify-form-{{ $menu->id }}').submit(); })">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <!-- Report -->
                                    @if(auth()->user()->hasPermission('courses.view'))
                                    <a href="{{ route('meal-menus.report', $menu->id) }}" 
                                    ...
                                    <!-- Edit -->
                                    @if(auth()->user()->hasPermission('courses.edit') && $menu->meal_date >= today())
                                    <a href="{{ route('meal-menus.edit', $menu->id) }}" 
                                    ...
                                    <form method="POST" action="{{ route('meal-menus.destroy', $menu->id) }}" class="inline" id="delete-form-{{ $menu->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" 
                                                class="p-2 text-red-600 hover:bg-red-50 rounded transition-colors"
                                                title="Eliminar"
                                                onclick="showConfirmModal('¿Estás seguro de eliminar este menú? Esta acción no se puede deshacer.', function() { document.getElementById('delete-form-{{ $menu->id }}').submit(); })">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">

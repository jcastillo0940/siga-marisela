...
@section('page-title', 'Editar Programación de Curso')
...
<div class="fade-in">
            ...
            <ol class="inline-flex items-center space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('course-offerings.show', $offering->id) }}" class="text-gray-500 hover:text-accent-red transition-colors">
                            {{ $offering->full_name }}
                        </a>
                    </div>
                </li>
    ...
    <form method="POST" action="{{ route('course-offerings.update', $offering->id) }}" id="offering-form">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="card-premium">
                    <div class="flex items-center space-x-4 pb-6 border-b border-gray-100">
                        <div class="w-16 h-16 sm:w-20 sm:h-20 bg-indigo-600 rounded-full flex items-center justify-center text-white text-2xl sm:text-3xl font-display font-semibold">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 mt-1">{{ $offering->code }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-premium">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                            ...
                            <select id="course_id" 
                                ...
                                @foreach($courses as $course)
                                    <option value="{{ $course->id }}" 
                                            ...
                                            {{ old('course_id', $offering->course_id) == $course->id ? 'selected' : '' }}>
                        ...
                        <div class="md:col-span-2">
                            <label class="flex items-center space-x-3">
                                <input type="checkbox" 
                                       ...
                                       id="is_generation"
                                       value="1"
                                       {{ old('is_generation', $offering->is_generation) ? 'checked' : '' }}
                        ...
                        <!-- Generation Name -->
                        <div class="md:col-span-2" id="generation-name-container" style="display: {{ old('is_generation', $offering->is_generation) ? 'block' : 'none' }};">
                            <label for="generation_name" class="label-elegant">Nombre de la Generación</label>
                            <input type="text" 
                                   id="generation_name" 
                                   name="generation_name" 
                                   value="{{ old('generation_name', $offering->generation_name) }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" 
                                   id="location" 
                                   name="location" 
                                   value="{{ old('location', $offering->location) }}"
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   id="duration_hours" 
                                   name="duration_hours" 
                                   value="{{ old('duration_hours', $offering->duration_hours) }}"
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   id="min_students" 
                                   name="min_students" 
                                   value="{{ old('min_students', $offering->min_students) }}"
                    ...
                    <div class="mt-4 sm:mt-6">
                        ...
                        <textarea id="notes" 
                                  name="notes" 
                                  rows="3"
                                  class="input-elegant @error('notes') border-red-500 @enderror">{{ old('notes', $offering->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="card-premium">
                    ...
                    <div id="class-dates-container" class="space-y-4">
                        @foreach($offering->dates as $index => $date)
                        <div class="border border-gray-200 rounded p-4 bg-neutral-bg" id="class-date-{{ $index }}">
                            <div class="flex items-center justify-between mb-3">
                                <h4 class="font-medium text-primary-dark">Clase #{{ $index + 1 }}</h4>
                                <button type="button" 
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input type="date" 
                                           ...
                                           value="{{ $date->class_date->format('Y-m-d') }}"
                                           class="input-elegant" 
                                           required>
                                </div>
                                <div>
                ...
                <div class="card-premium">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <button type="button" 
                                    onclick="showConfirmModal('¿Estás seguro de eliminar esta programación? Esta acción no se puede deshacer.', function() { document.getElementById('delete-form').submit(); })"
                        ...
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                            <a href="{{ route('course-offerings.show', $offering->id) }}" class="btn-secondary text-center">
                                Cancelar
                            </a>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <select id="status" 
                                ...
                                <option value="programado" {{ old('status', $offering->status) == 'programado' ? 'selected' : '' }}>Programado</option>
                                <option value="en_curso" {{ old('status', $offering->status) == 'en_curso' ? 'selected' : '' }}>En Curso</option>
                                <option value="completado" {{ old('status', $offering->status) == 'completado' ? 'selected' : '' }}>Completado</option>
                                <option value="cancelado" {{ old('status', $offering->status) == 'cancelado' ? 'selected' : '' }}>Cancelado</option>
                            </select>
                        </div>
                        ...
                        <div>
                            <label class="flex items-center space-x-3">
                                <input type="checkbox" 
                                       ...
                                       value="1"
                                       {{ old('certificate_included', $offering->certificate_included) ? 'checked' : '' }}
                                       class="w-4 h-4 sm:w-5 sm:h-5 text-accent-red border-gray-300 rounded focus:ring-accent-red">
                ...
                <div class="card-premium">
                    ...
                    <div class="space-y-3 text-xs sm:text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="text-primary-dark font-medium">{{ $offering->enrollments->count() }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Cupos disponibles:</span>
                            <span class="text-primary-dark font-medium">{{ $offering->available_spots }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Clases programadas:</span>
                            <span class="text-primary-dark font-medium">{{ $offering->dates->count() }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Creado:</span>
                            <span class="text-primary-dark font-medium">{{ $offering->created_at->format('d/m/Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <form id="delete-form" 
          method="POST" 
          action="{{ route('course-offerings.destroy', $offering->id) }}"
          class="hidden">
        @csrf
        @method('DELETE')
    </form>
...
@push('scripts')
<script>
let classDateIndex = {{ $offering->dates->count() }};

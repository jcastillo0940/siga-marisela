...
<body class="bg-gray-50">
        ...
        <div class="w-2/3 flex flex-col bg-white">
            ...
            <div class="bg-gradient-to-r from-red-900 to-red-700 text-white p-4 flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="font-bold text-lg"><?php echo e($activeCashRegister->code); ?></p>
                    </div>
                </div>
                <div class="flex items-center gap-4">
                    <div class="text-right">
                        <p class="text-sm opacity-90">Fondo Inicial</p>
                        <p class="font-bold text-xl"><?php echo e($activeCashRegister->formatted_opening_amount); ?></p>
                    </div>
                    <a href="<?php echo e(route('dashboard')); ?>" class="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg font-semibold transition">
                        ← Salir
                    </a>
                </div>
            </div>
            ...
            <div id="products-tab" class="flex-1 overflow-y-auto p-4">
                <div class="grid grid-cols-4 gap-4">
                    ...
                    <button onclick='addToCart({type:"product",id:<?php echo e($product->id); ?>,name:"<?php echo e(addslashes($product->name)); ?>",price:<?php echo e($product->price); ?>,track_inventory:<?php echo e($product->track_inventory ? "true" : "false"); ?>,stock:<?php echo e($product->stock ?? 0); ?>})' 
                        ...
                        <p class="text-2xl font-bold text-red-600">$<?php echo e(number_format($product->price, 2)); ?></p>
                        <?php if($product->track_inventory): ?>
                        <p class="text-xs text-gray-500 mt-1">Stock: <?php echo e($product->stock); ?></p>
                        <?php endif; ?>
                    </button>
            ...
            <div id="payments-tab" class="flex-1 overflow-y-auto p-4 hidden">
                <?php if($student && $enrollments->isNotEmpty()): ?>
                <div class="space-y-4">
                    <div class="bg-gradient-to-r from-green-50 to-blue-50 border-2 border-green-300 rounded-xl p-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <h3 class="text-xl font-bold"><?php echo e($student->full_name); ?></h3>
                                    <p class="text-sm text-gray-600"><?php echo e($student->identification); ?></p>
                                </div>
                            </div>
                            <a href="<?php echo e(route('pos.unified')); ?>" class="text-red-700 hover:text-red-900 font-semibold">✕ Cambiar</a>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border-2 border-gray-200 rounded-xl p-4">
                        <div class="flex justify-between mb-3 pb-3 border-b">
                            <h4 class="font-bold text-lg"><?php echo e($enrollment->courseOffering->course->name); ?></h4>
                            <p class="text-2xl font-bold text-red-700">$<?php echo e(number_format($enrollment->paymentPlan->balance, 2)); ?></p>
                        </div>
                        ...
                        <div class="p-3 border-2 rounded-lg cursor-pointer hover:border-green-500 transition mb-2 <?php echo e($schedule->is_overdue ? 'bg-red-50 border-red-300' : 'bg-blue-50 border-blue-200'); ?>"
                             onclick='addToCart({type:"payment",enrollment_id:<?php echo e($enrollment->id); ?>,schedule_id:<?php echo e($schedule->id); ?>,installment:<?php echo e($schedule->installment_number); ?>,course_name:"<?php echo e(addslashes($enrollment->courseOffering->course->name)); ?>",amount:<?php echo e($schedule->balance); ?>,due_date:"<?php echo e($schedule->due_date->format('d/m/Y')); ?>",is_overdue:<?php echo e($schedule->is_overdue ? "true" : "false"); ?>})'>
                            <div class="flex justify-between items-center">
                                <div>
                                    <span class="px-3 py-1 rounded-full text-sm font-bold <?php echo e($schedule->is_overdue ? 'bg-red-600 text-white' : 'bg-blue-600 text-white'); ?>">Cuota #<?php echo e($schedule->installment_number); ?></span>
                                    <?php if($schedule->is_overdue): ?><span class="ml-2 text-xs bg-red-600 text-white px-2 py-1 rounded font-bold">⚠ VENCIDA</span><?php endif; ?>
                                    <p class="text-sm text-gray-600 mt-1">Vence: <?php echo e($schedule->due_date->format('d/m/Y')); ?></p>
                                </div>
                                <p class="text-2xl font-bold">$<?php echo e(number_format($schedule->balance, 2)); ?></p>
                            </div>
                        </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
    ...
    <script>
        ...
        function processPayment() {
            ...
            form.action = hasProducts ? '<?php echo e(route("sales.store")); ?>' : '<?php echo e(route("payments.store")); ?>';
            form.submit();
        }
        ...
        document.getElementById('universal-search').addEventListener('input', function(e) {
            ...
            searchTimeout = setTimeout(async () => {
                const response = await fetch(`<?php echo e(route('pos.unified.search')); ?>?q=${encodeURIComponent(query)}`);
                ...
                if (data.students.length) {
                    ...
                    data.students.forEach(s => {
                        html += `<a href="<?php echo e(route('pos.unified')); ?>?student_id=${s.id}" class="block p-3 hover:bg-gray-50 border-b"><p class="font-bold">${s.icon} ${s.name}</p><p class="text-sm text-gray-600">${s.identification}</p></a>`;
                    });
                }
...
</html><?php /**PATH /home/u545703374/domains/test.supercarnes.com/resources/views/pos/unified.blade.php ENDPATH**/ ?>
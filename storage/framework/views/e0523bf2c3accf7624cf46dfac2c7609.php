<aside id="sidebar" 
    ...
    <nav class="flex-1 px-4 py-6 sm:py-8 space-y-1 overflow-y-auto">
        ...
        <?php if(auth()->user()->hasRole('student')): ?>
            ...
            <a href="<?php echo e(route('student-dashboard.root')); ?>"
            ...
            <a href="<?php echo e(route('public.leads.create')); ?>"
        ...
        <?php else: ?>
            ...
            <div class="accordion-menu">
                <button onclick="toggleAccordion('gestion-menu')" type="button"
                        class="w-full flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 rounded-sm transition-all duration-300 hover:bg-white hover:bg-opacity-5
                               <?php echo e(request()->routeIs('users.*') || request()->routeIs('students.*') || request()->routeIs('leads.*') || request()->routeIs('courses.*') || request()->routeIs('course-offerings.*') || request()->routeIs('enrollments.*') ? 'bg-white bg-opacity-10' : ''); ?>">
                    ...
                    <svg id="gestion-menu-icon" class="w-4 h-4 transition-transform duration-200 <?php echo e(request()->routeIs('users.*') || request()->routeIs('students.*') || request()->routeIs('leads.*') || request()->routeIs('courses.*') || request()->routeIs('course-offerings.*') || request()->routeIs('enrollments.*') ? 'rotate-180' : ''); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>
                <div id="gestion-menu" class="accordion-content <?php echo e(request()->routeIs('users.*') || request()->routeIs('students.*') || request()->routeIs('leads.*') || request()->routeIs('courses.*') || request()->routeIs('course-offerings.*') || request()->routeIs('enrollments.*') ? '' : 'hidden'); ?> ml-4 sm:ml-8 mt-1 space-y-1">
                    <?php if(auth()->user()->hasPermission('leads.view')): ?>
                        <a href="<?php echo e(route('leads.index')); ?>" class="flex items-center px-4 py-2 rounded-sm text-xs hover:bg-white hover:bg-opacity-5 <?php echo e(request()->routeIs('leads.*') ? 'text-white font-bold' : 'text-red-100'); ?>">Leads / Potenciales</a>
                    <?php endif; ?>
                    <?php if(auth()->user()->hasPermission('students.view')): ?>
                        <a href="<?php echo e(route('students.index')); ?>" class="flex items-center px-4 py-2 rounded-sm text-xs hover:bg-white hover:bg-opacity-5 <?php echo e(request()->routeIs('students.*') ? 'text-white font-bold' : 'text-red-100'); ?>">Estudiantes</a>
                    <?php endif; ?>
                    <?php if(auth()->user()->hasPermission('courses.view')): ?>
                        <a href="<?php echo e(route('courses.index')); ?>" class="flex items-center px-4 py-2 rounded-sm text-xs hover:bg-white hover:bg-opacity-5 <?php echo e(request()->routeIs('courses.*') ? 'text-white font-bold' : 'text-red-100'); ?>">Catálogo de Cursos</a>
            ...
            <div class="accordion-menu">
                <button onclick="toggleAccordion('admin-menu')" type="button"
                        class="w-full flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 rounded-sm transition-all duration-300 hover:bg-white hover:bg-opacity-5
                               <?php echo e(request()->routeIs('users.*') || request()->routeIs('roles.*') ? 'bg-white bg-opacity-10' : ''); ?>">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 sm:mr-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <svg id="admin-menu-icon" class="w-4 h-4 transition-transform duration-200 <?php echo e(request()->routeIs('users.*') || request()->routeIs('roles.*') ? 'rotate-180' : ''); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>
                <div id="admin-menu" class="accordion-content <?php echo e(request()->routeIs('users.*') || request()->routeIs('roles.*') ? '' : 'hidden'); ?> ml-4 sm:ml-8 mt-1 space-y-1">
                    <?php if(auth()->user()->hasPermission('users.view')): ?>
                        <a href="<?php echo e(route('users.index')); ?>" class="flex items-center px-4 py-2 rounded-sm text-xs hover:bg-white hover:bg-opacity-5 <?php echo e(request()->routeIs('users.*') ? 'text-white font-bold' : 'text-red-100'); ?>">Usuarios del Sistema</a>
                    <?php endif; ?>
                    <?php if(auth()->user()->hasPermission('roles.view')): ?>
                        <a href="<?php echo e(route('roles.index')); ?>" class="flex items-center px-4 py-2 rounded-sm text-xs hover:bg-white hover:bg-opacity-5 <?php echo e(request()->routeIs('roles.*') ? 'text-white font-bold' : 'text-red-100'); ?>">Roles y Permisos</a>
                    <?php endif; ?>
                </div>
            </div>
...
</script><?php /**PATH /home/u545703374/domains/test.supercarnes.com/resources/views/layouts/partials/sidebar.blade.php ENDPATH**/ ?>